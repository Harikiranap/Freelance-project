...
export default function Dashboard() {
  ...
  const handleAcceptBid = async (bidId, amount) => {
    ...
    try {
      const token = user?.token || localStorage.getItem('token');
      await axios.post(`http://localhost:5000/api/jobs/bid/${bidId}/accept`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      
      toast.success(`Bid accepted successfully! Agreed amount: ₹${amount}`);
      setViewingBidsJob(null);
      fetchJobs();
      fetchMyJobs();
    } catch (err) {
  ...
  const handleDeliverJob = async (jobId) => {
    if (!window.confirm("Are you sure you want to submit your final work for review?")) return;
    setIsSubmittingWork(jobId);
    try {
      const token = user?.token || localStorage.getItem('token');
      await axios.post(`http://localhost:5000/api/jobs/${jobId}/deliver`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleReleasePayment = async (jobId) => {
    if (!window.confirm("Are you satisfied with the work? This will release all secure escrow funds directly to the freelancer's account.")) return;
                  ...
                  <div className="mt-6 pt-5 border-t border-slate-100 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      {user?.role === 'client' && job.status === 'in-progress' && job.paymentStatus === 'pending' && (
                        <button
                          onClick={() => handlePayEscrow(job._id)}
                          disabled={isPaying === job._id}
                          className="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-lg text-xs font-bold shadow-md hover:shadow-lg transition-all flex items-center gap-1.5 disabled:opacity-50 animate-pulse hover:animate-none"
                        >
                          {isPaying === job._id ? <Loader2 size={12} className="animate-spin" /> : <CreditCard size={14} />}
                          Pay Escrow
                        </button>
                      )}
